...
Function Mute($mute){
    if($mute -eq $true){
        SoundSetGV "Mute" $true "$(LineNumber), fMuteTrue"
    }else{
        SoundSetGV "Mute" $false "$(LineNumber), fMuteFalse"
    }
}


Function Pause($pause){
    SoundSetGV "Pause" $pause "$(LineNumber), fpause"
}
...
Function MediaTest{
    param(
        ...
        SoundSetGV "WinLose" $songName "507, winloseJob"
...
#MUTE BOX
    ...
    #"$(Get-Date), $mute, $($muteCheckbox.Checked)" | Out-file "D:\MineSweeper\Sound\Config\MuteLog.txt" -Append
    Mute $mute
    ...
    -EventName Click `
    -Action {
        SoundSetGV "Mute" $($muteCheckbox.Checked) "$(LineNumber), optionsformEventAction";
        #Mute $mute;
        #"$(Get-Date), $($muteCheckbox.Checked)" | Out-file "D:\MineSweeper\Sound\Config\MuteLog.txt" -Append 
    }
...
#LOSE ACTION                
                If([int]$this.Name -in $bomb){
                    ...
                    Pause $true
                    Media $pathSet $imagePath "Bomb" "Bomb" #PLAY LOSE MUSIC AND IMAGE ##########################################1/10#####################################
                    ...
                    Do{Start-Sleep -Milliseconds 150}While($(SoundGetGV "MediaEnded") -eq $false)
                    Try{ #KILL IMAGE PID ONLY
                        $id = Get-WmiObject win32_process -filter "Name='powershell.exe' AND ParentProcessId=$PID" | Select ProcessID
                        $id.ProcessID | %{if($_ -ne $(SoundGetGV "PID")){Stop-Process -Id $_}} -ErrorAction Stop
                    }catch{}
                    Pause $false
                    $objForm.Activate();
...
#WIN ACTION                            
                    ...
                    if($script:score -ge $script:win){
                        ...
                        $script:winLose = "Win"
                        Pause $true
                        Media $pathSet $imagePath "Win" "Win" #PLAY WIN MUSIC AND IMAGE
                        #sleep -Seconds 6 #CAN THIS BE UPDATED TO USE END OF SONG EVENT? COMMENTED OUT 10/31/2020 replaced with DoWhile Loop checking for MediaEndedFlag
                        Do{Start-Sleep -Milliseconds 150}While($(SoundGetGV "MediaEnded") -eq $false)
                        Try{ #KILL IMAGE PID ONLY
                            $id = Get-WmiObject win32_process -filter "Name='powershell.exe' AND ParentProcessId=$PID" | Select ProcessID
...
#GetBombCount
    ...
    Import-Module "$PSScriptRoot\Sound\Modules\SoundGetGV\SoundGetGV.psm1" -Function SoundGetGV #ADDED 10/21/20 may need to adjust
    Import-Module "$PSScriptRoot\Sound\Modules\SoundSetGV\SoundSetGV.psm1" -Function SoundSetGV #ADDED 10/21/20 may need to adjust
...
#SCRIPTBLOCK TO RUN IN MEDIAPLAYER JOB
    $function = {
        function LineNumber {
            $($MyInvocation.ScriptLineNumber + 1576)
        } 
...
#FIFTH PART OF SCRIPT. LOGIC FOR CHANGING WMP INSTANCE CONTROL SETTINGS
            Function Change($pau,$mut,$nex,$son,$winLoseFlag,$me){
                "$(Get-Date), $(LineNumber), wmp-fChange-Top, $pau,$mut,$fir,$nex,$son" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                #IMPORT MODULES, GET CONFIG VALUES AND SET VARIABLES
                Import-Module "$using:path\Sound\Modules\SoundGetGV\SoundGetGV.psm1" -Function SoundGetGV
                Import-Module "$using:path\Sound\Modules\SoundSetGV\SoundSetGV.psm1" -Function SoundSetGV
                $Path=SoundGetGV "Path"
                $pause = SoundGetGV "Pause"
                $mute = SoundGetGV "Mute"
                $hold = SoundGetGV "Hold"
...
#1
                If($pau -eq 1 -and $winLoseFlag -eq 0 -and $nex -eq 1 -and $mute -eq 0){
                    SoundSetGV "Hold" 1
                    $Player.Pause()
                    SoundSetGV "PausedSongName" $song "$(LineNumber), wmp-fChange-pauseMusic"
                    SoundSetGV "PausedSongDuration" $($Player.Position.Ticks) "$(LineNumber), wmp-fChange-pauseMusic"
                    SoundSetGV "PausedSongPath" $($file.FullName) "$(LineNumber), wmp-fChange-pauseMusic"
                    "$(Get-Date), $(LineNumber), #1, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
...
#2
                If($pau -eq 1 -and $winLoseFlag -eq 0 -and $nex -eq 1 -and $mute -eq 1){
                    ...
                    SoundSetGV "PausedSongPath" $($file.FullName) "$(LineNumber), wmp-fChange-pauseMusic"
                    $Player.IsMuted=$true
                    "$(Get-Date), $(LineNumber), #2, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }
...
#3
                If($pau -eq 1 -and $winLoseFlag -eq 0 -and $nex -eq 1 -and $mute -eq 2){
                    ...
                    "$(Get-Date), $(LineNumber), #3, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }
...
#4
                If($pau -eq 1 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 0){
                    ...
                    SoundSetGV "PausedSongDuration" $($Player.Position.Ticks) "$(LineNumber), wmp-fChange-pauseMusic"
                    SoundSetGV "PausedSongPath" $($file.FullName) "$(LineNumber), wmp-fChange-pauseMusic"
                    "$(Get-Date), $(LineNumber), #4, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }
...
#5
                If($pau -eq 1 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 1){
                    ...
                    SoundSetGV "PausedSongPath" $($file.FullName) "$(LineNumber), wmp-fChange-pauseMusic"
                    $Player.IsMuted=$true
                    "$(Get-Date), $(LineNumber), #5, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }  
...
#6
                If($pau -eq 1 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 2){
                    ...
                    SoundSetGV "PausedSongPath" $($file.FullName) "$(LineNumber), wmp-fChange-pauseMusic"
                    $Player.IsMuted=$false
                    "$(Get-Date), $(LineNumber), #6, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }
...
#7
                If($pau -eq 2 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 0){
                    ...
                    SoundSetGV "MediaEnded" $false "$(LineNumber), wmp-fChange-UnPause"
                    SoundSetGV "SongChanged" $false "$(LineNumber), wmp-fChange-UnPause"
                    $Player.Play()
                    "$(Get-Date), $(LineNumber), #7, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }  
...
#8
                If($pau -eq 2 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 1){
                    ...
                    $Player.Play()                    
                    $Player.IsMuted=$true
                    "$(Get-Date), $(LineNumber), #8, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }  
...
#9
                If($pau -eq 2 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 2){
                    ...
                    $Player.Play()
                    $Player.IsMuted=$false
                    "$(Get-Date), $(LineNumber), #9, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }  

#10
                If($pau -eq 0 -and $winLoseFlag -eq 1 -and $nex -eq 1 -and $mute -eq 0){
                    SoundSetGV "Hold" 1
                    switch (SoundGetGV "WinLose"){
                        Bomb {$file = (get-item "$using:path\Sound\Sounds\Bomb.wav")}
                        Win  {$file = (get-item "$using:path\Sound\Sounds\winTrumpets.wav")}
                        Yankee   {$file = (get-item "$using:path\Sound\Sounds\Yankee-Doodle-Dandy.wav")}
                        Ultimate   {$file = (get-item "$using:path\Sound\Sounds\Ultimate-Victory.wav")}
                        Surrounded {$file = (get-item "$using:path\Sound\Sounds\Surrounded-by-the-Enemy.wav")}
	                    #n1  {$file = (get-item "")}
	                    default {
                    ...
                    $Player.Play()
                    "$(Get-Date), $(LineNumber), #10, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }  
...
#11
                If($pau -eq 0 -and $winLoseFlag -eq 1 -and $nex -eq 1 -and $mute -eq 1){
                    ...
                    $Player.Play()
                    $Player.IsMuted=$true
                    "$(Get-Date), $(LineNumber), #11, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }  
...
#12
                If($pau -eq 0 -and $winLoseFlag -eq 1 -and $nex -eq 1 -and $mute -eq 2){
                    ...
                    $Player.Play()
                    $Player.IsMuted=$false
                    "$(Get-Date), $(LineNumber), #12, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }  
                

#13
                If($pau -eq 0 -and $winLoseFlag -eq 1 -and $nex -eq 0 -and $mute -eq 0){
                    switch (SoundGetGV "WinLose"){
                        Bomb {$file = (get-item "$using:path\Sound\Sounds\Bomb.wav")}
                        Win  {$file = (get-item "$using:path\Sound\Sounds\winTrumpets.wav")}
                        Yankee   {$file = (get-item "$using:path\Sound\Sounds\Yankee-Doodle-Dandy.wav")}
                    ...
                    $Player.Open($($file.FullName))
                    $Player.Play()
                    "$(Get-Date), $(LineNumber), #13, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }  
...
#14
                If($pau -eq 0 -and $winLoseFlag -eq 1 -and $nex -eq 0 -and $mute -eq 1){
                    ...
                    $Player.Play()
                    $Player.IsMuted=$true
                    "$(Get-Date), $(LineNumber), #14, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
...
#15
                If($pau -eq 0 -and $winLoseFlag -eq 1 -and $nex -eq 0 -and $mute -eq 2){
                    ...
                    $Player.Play()
                    $Player.IsMuted=$false
                    "$(Get-Date), $(LineNumber), #15, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }                          
                        
#16
                If($pau -eq 0 -and $winLoseFlag -eq 0 -and $nex -eq 1 -and $mute -eq 0){
                    $song = SoundGetGV "PlayList" $(SoundGetGV "Playing")
                    SoundSetGV "Song" $song "$(LineNumber), wmp-fChange-ChangeSong"
                    if($(SoundGetGV "Playing") -eq $(SoundGetGV "SongCount")){
                        SoundSetGV "Playing" 0 "$(LineNumber), wmp-fChange-ChangeSong"
                    }else{
                        SoundSetGV "Playing" $($(SoundGetGV "Playing") + 1) "$(LineNumber), wmp-fChange-ChangeSong"
                    }
                    switch ($song){
                        Bomb {$file = (get-item "$using:path\Sound\Sounds\Bomb.wav")}
                        Win  {$file = (get-item "$using:path\Sound\Sounds\winTrumpets.wav")}
                    ...
                    $Player.Open($($file.FullName))
                    SoundSetGV "MediaEnded" $false "$(LineNumber), wmp-fChange-SongEndorSongChange"
                    SoundSetGV "SongChanged" $false "$(LineNumber), wmp-fChange-SongEndorSongChange"
                    $Player.Play()
                    "$(Get-Date), $(LineNumber), #16, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }                          
...
#17
                If($pau -eq 0 -and $winLoseFlag -eq 0 -and $nex -eq 1 -and $mute -eq 1){
                    ...
                    $Player.Play()
                    $Player.IsMuted=$true
                    "$(Get-Date), $(LineNumber), #17, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }                          
...
#18
                If($pau -eq 0 -and $winLoseFlag -eq 0 -and $nex -eq 1 -and $mute -eq 2){
                    ...
                    $Player.Play()
                    $Player.IsMuted=$false
                    "$(Get-Date), $(LineNumber), #18, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }                          
...
#19
                If($pau -eq 0 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 0){
                    ...
                    if($(SoundGetGV "Playing") -eq $(SoundGetGV "SongCount")){
                        ...
                        SoundSetGV "Playing" $($(SoundGetGV "Playing") + 1) "$(LineNumber), wmp-fChange-ChangeSong" -Append
                    }
                    switch ($song){
                        Bomb {$file = (get-item "$using:path\Sound\Sounds\Bomb.wav")}
                        Win  {$file = (get-item "$using:path\Sound\Sounds\winTrumpets.wav")}
                    ...
                    SoundSetGV "SongChanged" $false "$(LineNumber), wmp-fChange-SongEndorSongChange"
                    $Player.Play()
                    "$(Get-Date), $(LineNumber), #19, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append                    
                }                      
                    
#20
                If($pau -eq 0 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 1){
                    $Player.IsMuted=$true
                    "$(Get-Date), $(LineNumber), #20, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }                      
                    
#21
                If($pau -eq 0 -and $winLoseFlag -eq 0 -and $nex -eq 0 -and $mute -eq 2){
                    $Player.IsMuted=$false
                    "$(Get-Date), $(LineNumber), #21, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }                      
                    
#22                    
                if($pau -eq $null -or $winLoseFlag -eq $null -or $nex -eq $null -or $mute -eq $null){
                    "$(Get-Date), $(LineNumber), #22, $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                }    
                        
                                                            

                #OLD WMP LOGIC - Partly works
                <#
                    If($mut -ne 1){
                        #CHECK IF NOT WINLOSE
                        If($(SoundGetGV "WinLose") -eq ""){
                
                            #MUSIC IS PLAYING NOT PAUSED, BUT NOT UNPAUSED. CHANGES SONG
                            ...
                            #MUSIC PAUSED OR MUSIC UNPAUSED. RELEASES UNPAUSED FLAG.
                            }else{
                                SoundSetGV "UnPaused" $false "$(LineNumber), wmp-fChange-mPausedorUnpaused"
                            }

                            #PAUSE MUSIC, CAPTURE SONG PAUSE POSITION. MAY NEED TO ENSURE I'M SAVING THE CORRECT INFO FOR PAUSE. 
                            if($pause -eq $true){
                                $Player.Pause()
                                switch ($song){
                                    Bomb {$file = (get-item "$using:path\Sound\Sounds\Bomb.wav")}
                            ...
                            #IF NOT PAUSED
                                ...
                                #IF SONG ENDED OR SONG CHANGED, PLAY ANOTHER SONG AND NOT PAUSED
                    ...
                    }elseif($mut -eq 1 -and $(SoundGetGV "MediaEnded") -eq $true){
                            switch (SoundGetGV "WinLose"){
                                Bomb {$file = (get-item "$using:path\Sound\Sounds\Bomb.wav")}
                                Win  {$file = (get-item "$using:path\Sound\Sounds\winTrumpets.wav")}
                                Yankee   {$file = (get-item "$using:path\Sound\Sounds\Yankee-Doodle-Dandy.wav")}
        ...
        #> #END OF OLD WMP LOGIC                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        

                Return {[ref]$player; [ref]$mediaEnded; [ref]$mut}
            } #End Function Change

#THIRD PART OF THE SCRIPT. INSTANTIATES VARIABLES, SETS CONFIG FILE VALUES
            Import-Module "$using:path\Sound\Modules\SoundGetGV\SoundGetGV.psm1" -Function SoundGetGV
            Import-Module "$using:path\Sound\Modules\SoundSetGV\SoundSetGV.psm1" -Function SoundSetGV
            SoundSetGV "PlayList" $("Yankee","Ultimate","Surrounded" | Sort-Object {Get-Random}) "$(LineNumber), wmp-p3-instantiatevarsPlayList"
            SoundSetGV "SongCount" $($(SoundGetGV "PlayList").Count - 1) "$(LineNumber), wmp-p3-instantiatevarsSongCount"
            SoundSetGV "Playing" 0 "$(LineNumber), wmp-p3-instantiatevarsPlaying"
            SoundSetGV "UnPaused" $false "$(LineNumber), wmp-p3-instantiatevarsUnPaused"
            SoundSetGV "PausedSongPath" "" "$(LineNumber), wmp-p3-instantiatevarsPausedSongPath"
            SoundSetGV "PausedSongDuration" $(
                                   ...
                                   '"TotalMinutes":  0,',
                                   '"TotalSeconds":  0'
            ) "$(LineNumber), wmp-p3-instantiatevarsPausedSongDuration"
            SoundSetGV "PausedSongName" "" "$(LineNumber), wmp-p3-instantiatevarsPausedSongName"
            SoundSetGV "WinLose" "" "$(LineNumber), wmp-p3-instantiatevarsWinLose"
            SoundSetGV "Mute" $false "" "$(LineNumber), wmp-p3-instantiatevarsMute"
            $pau=$false
            $winLoseFlag=0
            $nex=0
            $mut=0
...
#FOURTH PART OF WMP SCRIPT. LOOPS TO CHECK FOR CHANGES TO MEDIA. CALLS CHANGE FUNCTION IF CHANGES ARE FOUND 
                   $i = $true
        :doloop    do{
                        #CHECK IF SOUNDCONFIG.TXT CHANGED SINCE LAST RUN. USING VARIABLES TO CHECK AGAINST AND SLEEP.
                        if($($pause -ne $(SoundGetGV "Pause")) -or $(SoundGetGV "MediaEnded") -eq $true -or $($mute -ne $(SoundGetGV "Mute")) -or $song -ne $(SoundGetGV "Song") -or $winLose -ne $(SoundGetGV "WinLose")){
                            if($pause -ne $(SoundGetGV "Pause")){$pau=1; if($(SoundGetGV "Pause") -eq $false){SoundSetGV "UnPaused" $true "$(LineNumber), wmp-p4-insideIfDoLoop"}}
                            if($mute -ne $(SoundGetGV "Mute")){$mut=1}
                            if($song -ne $(SoundGetGV "Song")){SoundSetGV "SongChanged" $true "$(LineNumber), wmp-p4-insideIfDoLoop"}
                            if($winLose -ne $(SoundGetGV "WinLose")){$winLoseFlag=1}
                            if($(SoundGetGV "MediaEnded") -eq $true){$me=1}
                            if($pau -eq $null -or $winLoseFlag -eq $null -or $nex -eq $null -or $mute -eq $null){
                                if($pau -eq $null){$pau=0}
                                if($winLoseFlag -eq $null){$winLoseFlag=0}
                                if($nex -eq $null){$nex=0}
                                if($mute -eq $null){$mute=0}
                                "$(Get-Date), $(LineNumber), $pau, $winLoseFlag, $nex, $mute" | Out-File "D:\MineSweeper\Sound\Config\SongLog.txt" -Append
                            }  
                            Change $pau $mut $nex $son $winLoseFlag $me
                    
                        #NO CHANGES YET
                        }else{}

                        #FILL VARIABLES BEFORFE SLEEP (SLEEP CREATES GAP TO CHECK FOR DIFFERENCES AFTERWARDS)
                        $pause = SoundGetGV "Pause"
                        $mute = SoundGetGV "Mute"
                        $mediaEnded = SoundGetGV "MediaEnded" # Is this supposed to be filled by the function return?
                        $song = SoundGetGV "Song"
                        $winLose = SoundGetGV "WinLose"
                        if($player.Position.Ticks -eq $player.NaturalDuration.TimeSpan.Ticks){
                            if($(SoundGetGV "WinLose") -ne ""){
                                SoundSetGV "WinLose" "" "$(LineNumber), wmp-p4-SongEndedWinLose"
                                SoundSetGV "Pause" $false "$(LineNumber), wmp-p4-SongEndedWinLose"
                            }else{
                                $mediaEnded=$true;
                                SoundSetGV "MediaEnded" $true "$(LineNumber), wmp-p4-SongEndedNotWinLose"
                                $mut = $null
                            }
...
#2ND PART OF THE WMP SCRIPT. CREATES WMP INSTANCE
        ...
        SoundSetGV "Pause" $false "$(LineNumber), wmp-p2-Creates WMP Instance"
        SoundSetGV "MediaEnded" $true "$(LineNumber), wmp-p2-Creates WMP Instance"
        Sound
    }
...
#1ST PART OF THE WMP SCRIPT BEGINS AT BEGINNING OF GAME. REMOVES PREVIOUS RAN JOBS, CAPTURES SCRIPTROOT, REMOVE PREVIOUS VERSIONS OF MODULE, IMPORTS MODULE, SETS PATH TO CONFIG, RUNS SCRIPTBLOCK AS JOB.
    ...
    $script:path=(get-item $PSScriptRoot).FullName
    ...
    Import-Module "$path\Sound\Modules\SoundSetGV\SoundSetGV.psm1" -Function SoundSetGV -Scope Global
    SoundSetGV "Path" $path "$(LineNumber), wmp-p1-RunJob"
    SoundSetGV "Play" 1 "$(LineNumber), wmp-p1-RunJob"
    ...
    SoundSetGV "PID" $((Get-WmiObject win32_process -filter "Name='powershell.exe' AND ParentProcessId=$PID").ProcessID) "$(LineNumber), wmp-p1-RunJob"

    #THIS EVENT MAY BE USED TO RESTART WMP IF IT STOPS FOR SOME REASON
    Get-EventSubscriber -Force | Unregister-Event -Force
    #Get-Event | Remove-Event
    $null = Register-ObjectEvent -SourceIdentifier 1 -InputObject $musicJob -EventName StateChanged -Action {$music = 0;write-host $music; SoundSetGV "Play" 0 "$(LineNumber), wmp-p1-RegEvent"}
