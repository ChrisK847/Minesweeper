...
#LOSE ACTION                
                If([int]$this.Name -in $bomb){
                    ...
                    Pause $true
                    Media $pathSet $imagePath "Bomb" "Bomb" #PLAY LOSE MUSIC AND IMAGE
                    sleep -Seconds 6 #CAN THIS BE UPDATED TO USE END OF SONG EVENT?
...
#THIRD PART OF THE SCRIPT. INSTANTIATES VARIABLES, SETS CONFIG FILE VALUES
            ...
            SoundSetGV "PlayList" $("Yankee","Ultimate","Surrounded" | Sort-Object {Get-Random}) "$(LineNumber), wmp-p3-instantiatevars"
            SoundSetGV "SongCount" $($(SoundGetGV "PlayList").Count - 1) "$(LineNumber), wmp-p3-instantiatevars"
            SoundSetGV "Playing" 0 "$(LineNumber), wmp-p3-instantiatevars"
            SoundSetGV "UnPaused" $false "$(LineNumber), wmp-p3-instantiatevars"
            SoundSetGV "PausedSongPath" "" "$(LineNumber), wmp-p3-instantiatevars"
            SoundSetGV "PausedSongDuration" $(
                                   ...
                                   '"TotalMinutes":  0,',
                                   '"TotalSeconds":  0'
            ) "$(LineNumber), wmp-p3-instantiatevars"
            SoundSetGV "PausedSongName" "" "$(LineNumber), wmp-p3-instantiatevars"
            SoundSetGV "WinLose" "" "$(LineNumber), wmp-p3-instantiatevars"
